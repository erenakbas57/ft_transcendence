export function PongTournament() {
    ...
    let player1MovingUp = false;  // Sol oyuncu
    let player1MovingDown = false;
    let player2MovingUp = false;  // Sağ oyuncu
    ...
    function applyTheme(theme) {
        if (theme === "football") {
            ...
            canvas.style.backgroundColor = "#B4CAE0";
        }
    }
    ...
    function update() {
        ...
        if (playerLScore >= maxRounds || playerRScore >= maxRounds) {
            totalMatch++;
            if (totalMatch >= 3)
                endGame();
            else
    ...
    function draw() {
        ...
        ctx.fillStyle = 'white';
        ctx.fillRect(0, player1Y, paddleWidth, paddleHeight);  // Sol oyuncu
        ctx.fillRect(canvas.width - paddleWidth, player2Y, paddleWidth, paddleHeight);  // Sağ oyuncu
